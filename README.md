...
**[TarkovBright.exe — последний релиз, ~11 МБ](https://github.com/markuzewb/tarkovLight/releases/latest/download/TarkovBright.exe)**

Скачал → положил в любую папку → двойной клик. Ни Python, ни `pip`, ни распаковки
репозитория: всё нужное уже внутри одного файла (то же самое окно, те же хоткеи
F7–F10, те же настройки в `%APPDATA%\TarkovBright\config.json`).

* **Windows может спросить «Удалённый файл. Вы уверены?»** (SmartScreen) — файл не
  подписан коммерческим сертификатом, поэтому предупреждение будет. Правильный
  ответ: «Дополнительно → Выполнить в любом случае». Проверить, что файл не изменили
  в дороге, можно по SHA-256 — он опубликован в тексте релиза.
* **Антивирус** на PyInstaller-обёртки иногда ругается «возможное нежелательное ПО»:
  это ложняк на упаковщик, а не на код — исходники открыты, ничего в игру прога не
  внедряет (см. «Безопасность»).
...
* **Обновление.** `.exe` сам себя заменить не может, поэтому в окне вместо кнопки
  ...
  скажет «есть релиз 1.x — замените файл». Если хотите именно кнопку «Обновить» —
...
и подмена `app/*.py` рядом ничего не меняет; программа честно говорит об этом вместо
бесполезной кнопки. Для exe-варианта берите `TarkovBright.exe` из артефакта
`TarkovBright-exe` в GitHub Actions (сборка на каждый push в `main`) и просто заменяйте файл.

Программа запускается **в один экземпляр**: второй запуск при уже открытом окне
завершится сразу с объяснением (два цикла, крутящих одну gamma-таблицу, только воюют
между собой). Нужны оба — `--force-multi`.
...
проверка гамма-таблицы: ПРОБЛЕМА — SetDeviceGammaRamp вернул отказ — код Windows 87: ...
  ...
  сеанс: УДАЛЁННЫЙ (RDP) | цвет: 32 бит/пиксель, 1 планов (точность канала по GDI не видна) | HDR вкл
...
.github/workflows/ CI: 6 наборов тестов + --selftest на ubuntu и
                   windows (3.10/3.12), GUI под Xvfb, прогон в cp1252, артефакт exe.
                   release.yml: на тег v* собирает TarkovBright.exe и выкладывает
                   его в GitHub Release — та самая «скачать один файл»
