...
*левая колонка — как видит Тарков, правая — после авто-коррекции; цифры в
[README ниже](#настройка-под-себя-без-игры) воспроизводятся `tools/preview.py`.*
...
1. Поставить Python 3.9+ с галкой **Add python.exe to PATH**
   (или собрать `TarkovBright.exe` через `Build-exe.bat`). Один раз:
   `winget install -e --id Python.Python.3.12`.
2. Запустить **`install.bat`**. Он делает три вещи и ни одна не обязательна для
   запуска: находит Python, который **реально запускается** (не `where py`!),
   пробует поставить опциональные `numpy`/`mss`/`Pillow` и прогонит
   `--selftest` + `--check`. Если pip нет — скажет «skip» и всё равно заработает.
3. Запустить **`Start-TarkovBright.bat`** → появится окно с ползунками.
...
python tests/test_gui.py           # GUI-смоук (скипается без дисплея)
```
...
| реальный код захвата (pillow-бэкенд) и даунскейл 960×540 → 560×315 | вид окна: tkinter в песочнице без `DISPLAY` не поднять (смоук `test_gui.py` это честно скипает; логика очереди/ползунков проверена тестами) |
...
```
app/main.py        окно, хоткеи, поток, CLI (--headless --preview --check --restore --selftest --profile)
...
app/windows.py     ctypes: gamma-таблица, GetAsyncKeyState, HDR-проба, поиск игры, яркость панели
...
.github/workflows/ CI: 4 набора тестов + --selftest на ubuntu и windows (3.10/3.12)
install.bat        находит РАБОТАЮЩИЙ python, опционально ставит numpy/mss/Pillow
