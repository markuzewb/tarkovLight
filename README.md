...
## Если «не работает»
### «Двигаю ползунки — картинка не меняется»

Три независимых выключателя, и отказ любого из них выглядит одинаково («не реагирует»):

1. **Галка «Включено» (F8).** Без неё программа считает цифры и молчит. В v1.3.3 и
   раньше она могла сняться **сама**: приложение видело `SM_REMOTESESSION=1`, писало
   «сеанс RDP: программная gamma-таблица недоступна» и записывало `enabled: false` в
   `config.json` — даже когда проба только что успешно поставила таблицу. С v1.3.4
   флаг не имеет права отменять результат пробы: верим проверке, а не флагу.
2. **Привязка к игре.** Пока стоит галка «работать, только когда запущен Тарков»
   (`"tie_to_game": true` по умолчанию) и игры нет — эффект спит. Для проверки без
   игры: снять галку или `--always` (и то же самое советует `--doctor`).
3. **Таблицу реально не принимают.** Тогда в окне красной строкой написано почему
   («драйвер вернул FALSE…», «таблица ставится, но не применяется», «не приняли 3×256
   и 3×1024»), а в `--doctor` — строка «доказательство». См. два предыдущих раздела.

Отдельный случай для двух мониторов: `SetDeviceGammaRamp` действует на тот DC, который
мы получили. `GetDC(NULL)` — это первичный адаптер; с v1.3.3 проба при отказе первичного
перебирает DC каждого монитора и остаётся на том, где таблицу приняли (в отчёте —
«принято только на DC монитора …»). Если нужно, чтобы гамма легла сразу на все экраны,
скажите — это одна строчка конфигурации, но писать тогда приходится в каждый DC.
...
С v1.3.4 это ещё и **не приговор**: флаг больше не может отменить успешную пробу —
если таблица ставится, эффект работает, а про противоречие вы увидите только
пояснение в примечании окна.
