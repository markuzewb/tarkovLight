...
DEFAULT_CONFIG = {
    ...
    # «один файл .exe живёт в фоне»: закрытие окна НЕ выключает эффект — окно
    # сворачивается, а гамма и хоткеи продолжают работать; выход — кнопкой
    # «Выход» или F7. По умолчанию True (это поведение собранного .exe);
    # из исходников (.py/.pyw) main() принудительно ставит False — там закрытие
    # окна выключает всё, как было всегда. Конфиг-значение для .exe уважается.
    "minimize_on_close": True,
    "autostart": False,         # запускать при входе в Windows (реестр HKCU Run,
                                # имеет смысл только для собранного .exe)
}
